/// @func	generate_biome_temp({int} minlevel,{int} maxlevel,{int} zones);
/// @desc	Generates temperature for biomes.
function generate_biome_temp(_minlevel=1,_maxlevel=5,_zones=5)
{
	var _points;
	...
	for (var _gy = 0; _gy < map_size_y; _gy++)
	{
		for (var _gx = 0; _gx < map_size_x; _gx++)
		{
			...
			// Set tile
			tilemap_set_at_pixel(temp_tiles,_tile,_x,_y);
		}
	}
}